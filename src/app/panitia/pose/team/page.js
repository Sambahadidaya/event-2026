...
export default function AdminPoseTeam() {
    ...
    const fetchTeam = async () => {
        setLoading(true);
        const cached = localStorage.getItem('pose_team');
        const cacheTime = localStorage.getItem('pose_team_time');
        ...
        const { data } = await supabase.from('team').select('*').eq('type', 'pose').order('created_at', { ascending: false });
        if (data) {
            setTeam(data);
            localStorage.setItem('pose_team', JSON.stringify(data));
            localStorage.setItem('pose_team_time', Date.now().toString());
        }
        setLoading(false);
    };
    ...
    const handleRefresh = () => {
        localStorage.removeItem('pose_team');
        localStorage.removeItem('pose_team_time');
        fetchTeam();
        setSelectedIds([]);
        cancelEdit();
    };
    ...
    const handleAddOrUpdate = async (e) => {
        ...
        const payload = { title, content, type: 'pose' };
    ...
    const handleDelete = async (id) => {
        if (!confirm('Apakah Anda yakin ingin menghapus data tim ini secara permanen?')) return;
            ...
            <div className="bg-white dark:bg-gray-900 p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
                ...
                <form onSubmit={handleAddOrUpdate} className="space-y-5">
                    ...
                    <div>
                        ...
                        <textarea
                            required placeholder="Tuliskan anggota tim di sini..." rows="4" value={content} onChange={e => setContent(e.target.value)}
                    ...
                    <div className="flex justify-end">
                        <button type="submit" disabled={isSubmitting} className={`px-6 py-2.5 rounded-lg font-medium flex items-center gap-2 text-white shadow-sm transition-colors ${editingId ? 'bg-orange-500 hover:bg-orange-600 disabled:bg-orange-300' : 'bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400'}`}>
                            ...
                            {isSubmitting ? 'Menyimpan...' : (editingId ? 'Update Tim' : 'Publikasi Tim')}
                        </button>
                    </div>
                </form>
            </div>
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-white dark:bg-gray-900 text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-800">
                            <tr>
                                ...
                                <th className="px-6 py-4 font-medium w-1/3">Nama Tim</th>
                                ...
                                <tr>
                                    <td colSpan="4" className="px-6 py-12 text-center text-gray-500">
                                        <div className="flex flex-col items-center justify-center">
                                            ...
                                            <p className="text-sm mt-1">Data tabel tim kosong atau kata kunci tidak cocok.</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
