...
const CACHE_KEY = 'pose_register_cache';

export default function PoseRegisterDashboard() {
    ...
    // Reset nama lomba ketika jenis lomba berubah
    useEffect(() => {
        setNamaLomba('all');
    }, [jenisLomba]);
    ...
    const renderDetailFields = (item) => {
        ...
        return [
            ...
            { label: 'Kampus', value: item.team_members?.[0]?.kampus || '-' },
                    ...
                    <div className="mt-2 space-y-3">
                        {item.team_members?.map((m, idx) => (
                            <div key={idx} className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg text-sm border border-gray-100 dark:border-gray-700">
                                ...
                                <p className="text-gray-600 dark:text-gray-400">NIM: {m.nim || '-'} • Prodi: {m.prodi || '-'} • Angkatan: {m.angkatan || '-'}</p>
                                <p className="text-gray-600 dark:text-gray-400">Kampus: {m.kampus || '-'}</p>
                                <p className="text-gray-600 dark:text-gray-400">Kontak: {m.email_wa || '-'}</p>
                            </div>
                        ))}
                    </div>
                ),
    ...
    return (
        <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-500">
            <DashboardHeaderFilters
                ...
                subtitle="Kelola tim yang mendaftar lomba POSE"
                icon={Users}
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                            {loading && data.length === 0 ? (
                                ...
                                <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                    ...
                                    <td className="px-4 py-3 text-gray-600 dark:text-gray-400 text-xs">{item.team_members?.[0]?.kampus || '-'}</td>
