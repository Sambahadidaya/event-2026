...
import { toWibInputString, wibInputToIso, formatIndoDate } from '@/lib/dateUtils';
import { BookOpen, Plus, Edit2, Trash2, X, RefreshCw, Upload, FileText, Image as ImageIcon, Link as LinkIcon, Lock, Unlock } from 'lucide-react';
import { nanoid } from 'nanoid';

export default function AdminPjAcaraMateri() {
    ...
    const [statusMateri, setStatusMateri] = useState(false); // Draft/hidden
    const [statusTutup, setStatusTutup] = useState(false);   // Manual close
    ...
    const handleEdit = (item) => {
        ...
        setTanggal(toWibInputString(item.tanggal));
        setStatusMateri(Boolean(item.status));
        setStatusTutup(Boolean(item.tutup));
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const isoTanggal = wibInputToIso(tanggal);
            if (!isoTanggal) {
            ...
            const payload = {
                ...
                link_tugas: editingId ? (materiList.find(m => m.id === editingId)?.link_tugas || nanoid(10)) : nanoid(10)
            };
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Manajemen Materi PKKMB (PJ Acara)</h2>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Kelola file presentasi, jadwal waktu tayang WIB, status buka/tutup materi, dan tugas PKKMB.</p>
                </div>
                    ...
                    <form onSubmit={handleSubmit} className="p-6 space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Tanggal & Waktu Materi (WIB) *</label>
                                    ...
                                    <p className="text-xs text-gray-500 mt-1">*Waktu mengacu pada Waktu Indonesia Barat (WIB).</p>
                                </div>
                                
                                <div className="space-y-3 pt-2 bg-slate-50 dark:bg-slate-800/40 p-3.5 rounded-xl border border-slate-200/60 dark:border-slate-700/60">
                                    <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <input type="checkbox" id="tutup" checked={statusTutup} onChange={e => setStatusTutup(e.target.checked)} className="w-4 h-4 text-red-600 rounded border-gray-300" />
                                        <label htmlFor="tutup" className="text-sm font-semibold text-red-600 dark:text-red-400 cursor-pointer flex items-center gap-1.5">
                                            <Lock size={14} /> Tutup Materi Manual (Disable tombol Masuk Materi di publik)
                                        </label>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div key={item.id} className="bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col group relative">
                        <div className="h-32 bg-gray-200 dark:bg-gray-800 overflow-hidden relative">
                            ...
                            <div className="absolute top-2 right-2 flex items-center gap-1.5">
                                {item.status && <div className="bg-amber-500 text-white text-[10px] font-bold px-2 py-1 rounded-md">DRAFT</div>}
                                {item.tutup ? (
                                    <div className="bg-red-600 text-white text-[10px] font-bold px-2 py-1 rounded-md flex items-center gap-1">
                                        <Lock size={10} /> DITUTUP
                                    </div>
                                ) : (
                                    <div className="bg-emerald-600 text-white text-[10px] font-bold px-2 py-1 rounded-md flex items-center gap-1">
                                        <Unlock size={10} /> DIBUKA
                                    </div>
                                )}
                            </div>
                        </div>
