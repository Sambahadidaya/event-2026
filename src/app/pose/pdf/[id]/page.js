...
import { ShieldCheck, FileText, CheckCircle2, AlertTriangle, Building2, Calendar, User } from 'lucide-react';
import Link from 'next/link';

export async function generateMetadata({ params }) {
    const { id } = await params;
    return {
        title: `Verifikasi Dokumen #${id?.slice(0, 8) || ''} - POSE 2026`,
        description: 'Halaman resmi verifikasi keaslian dokumen digital Portal Kampus POSE 2026',
    };
}

export default async function PublicPosePdfPage({ params }) {
    const { id } = await params;
    const doc = await getDocumentById(id);

    const isNotFound = !doc;
    const primaryColorClass = 'from-blue-600 to-indigo-700';
    const badgeColorClass = 'bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300 border-blue-200 dark:border-blue-800';
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 py-10 px-4 flex flex-col items-center justify-center font-sans">
            <div className="max-w-xl w-full bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-slate-200/80 dark:border-slate-800 overflow-hidden">
                ...
                <div className={`bg-gradient-to-r ${primaryColorClass} p-6 sm:p-8 text-white text-center relative overflow-hidden`}>
                    ...
                    <p className="text-xs uppercase tracking-widest text-blue-100 font-bold mt-1">VERIFIKASI DOKUMEN DIGITAL POSE</p>
                </div>
                ...
                <div className="p-6 sm:p-8 space-y-6">
                        ...
                        <>
                            ...
                            <div className={`p-4 rounded-2xl border flex items-center justify-between ${badgeColorClass}`}>
                                <div className="flex items-center gap-3">
                                    <CheckCircle2 size={24} className="shrink-0 text-blue-600 dark:text-blue-400" />
                                    <div>
                    ...
                    <div className="pt-4 border-t border-slate-100 dark:border-slate-800 text-center text-xs text-slate-400 space-y-2">
                        <p>© 2026 Portal Kampus POSE - LP3I. Hak Cipta Dilindungi.</p>
                        <div>
                            <Link href="/pose" className="text-blue-600 dark:text-blue-400 hover:underline font-bold">
                                Kembali ke Website Utama POSE →
                            </Link>
                        </div>
                    </div>
                </div>
