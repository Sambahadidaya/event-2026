...
import { getWelcomeGuideConfig, shouldShowWelcomeGuide, markWelcomeGuideAsSeen } from '@/api/logic/welcomeGuideLogic';
import { shouldShowUpdatePopup } from '@/api/logic/updateVersionLogic';

export default function WelcomeGuideModal({ site }) {
    ...
    useEffect(() => {
        const guideData = getWelcomeGuideConfig(site, pathname);
        if (!guideData) {
            setIsOpen(false);
            setConfig(null);
            return;
        ...
        if (!shouldShowWelcomeGuide(site, pathname)) {
            setIsOpen(false);
            return;
        }
        ...
        // Jika ada UpdateVersionModal yang sedang aktif/akan tampil, tunggu sampai modal tersebut ditutup user
        if (shouldShowUpdatePopup(site)) {
            pollIntervalRef.current = setInterval(() => {
                if (!shouldShowUpdatePopup(site)) {
                    clearInterval(pollIntervalRef.current);
                    pollIntervalRef.current = null;
                    // Beri jeda halus setelah UpdateVersionModal ditutup
    ...
    const handleDismiss = () => {
        markWelcomeGuideAsSeen(site, pathname);
        setIsOpen(false);
    };
                ...
                <div className="p-5 sm:p-6 overflow-y-auto flex-1">
                    {hasVideo ? (
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-5 lg:gap-6 items-start">
                            {/* Video Guide Section (Desktop: 7 cols, Mobile: Top) */}
                            <div className="md:col-span-7 space-y-2 order-1">
                                <div className="flex items-center justify-between text-xs font-bold text-slate-700 dark:text-slate-300">
                                    ...
                                    <span className="text-[11px] font-normal text-slate-400">Putar langsung di sini</span>
                                </div>
                            ...
                            {/* Text Highlights Section (Desktop: 5 cols, Mobile: Bottom) */}
                        ...
                        /* Single Column Layout (Bila tidak ada video, misal PKKMB) */
