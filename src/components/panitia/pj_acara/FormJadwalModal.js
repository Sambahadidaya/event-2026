...
export default function FormJadwalModal({
    isOpen,
    onClose,
    onSave,
    editData = null
            ...
            <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden transform scale-100 transition-all duration-300">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800/80">
                    <h3 className="text-base font-bold text-slate-800 dark:text-white">
                        {editData ? 'Edit Jadwal Acara' : 'Tambah Jadwal Acara'}
                    </h3>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="space-y-1.5">
                        <label className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Judul Jadwal</label>
                        <input
                            type="text"
                            placeholder="Contoh: Pembukaan PKKMB 2026"
                            value={judul}
